...
def load_custom_stopwords():
    try:
        path = os.path.join(os.path.dirname(__file__), "data", "vietnamese-stopwords.txt")
...
def clean_query(query: str) -> str:
    ...
    words = query.split()
    # Lọc stop words
    cleaned = [w for w in words if w.lower() not in CUSTOM_STOP_WORDS]
    # Lọc tiếp các cụm 2 từ (như "làm ơn")
    text = " ".join(cleaned)
    for sw in CUSTOM_STOP_WORDS:
        if " " in sw:  # Cụm nhiều từ
