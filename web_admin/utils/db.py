...
def get_all_news(limit: int = 20, offset: int = 0, category: str = None) -> Tuple[List[Dict], int]:
    ...
    if category and has_category:
        cursor.execute(
            'SELECT COUNT(*) as cnt FROM articles WHERE link NOT LIKE %s AND category = %s',
            (UPLOAD_LINK_PATTERN, category),
        )
        ...
        cursor.execute('''
            ...
            FROM articles 
            WHERE link NOT LIKE %s AND category = %s
            ORDER BY published_date DESC 
            LIMIT %s OFFSET %s
        ''', (UPLOAD_LINK_PATTERN, category, limit, offset))
    else:
        cursor.execute(
        ...
        cursor.execute(f'''
            ...
            ORDER BY published_date DESC 
            LIMIT %s OFFSET %s
        ''', (UPLOAD_LINK_PATTERN, limit, offset))
...
def search_news(query: str, limit: int = 20) -> List[Dict]:
    ...
    cursor.execute('''
        ...
        ORDER BY published_date DESC 
        LIMIT %s
    ''', (UPLOAD_LINK_PATTERN, search_pattern, search_pattern, search_pattern, limit))
