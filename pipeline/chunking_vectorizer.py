...
class TextVectorizer:
    ...
    def __init__(self, model_name: str = 'bkai-foundation-models/vietnamese-bi-encoder'):
        ...
        print(f"[OK] Model ready! Dimension: {self._dim}")
...
def process_all_articles(incremental: bool = True, qdrant_service: Optional[QdrantService] = None, embedding_service: Optional[any] = None):
    """
    Xử lý bài báo: Semantic Chunking → Vectorization → Qdrant
    """
    print("\n[Process] STARTING CHUNKING & VECTORIZATION (QDRANT)")
    print("=" * 60)

    # 1. Sử dụng model đã có hoặc tải mới
    if embedding_service is not None:
        vectorizer = embedding_service
    else:
        from chatbot_api.dependencies import get_embedding_service
        vectorizer = get_embedding_service()
    ...
    processed_ids = set()
    if incremental:
        print("[Info] Checking existing data in Qdrant...")
        try:
            ...
            while True:
                ...
                if offset is None:
                    break
            print(f"[OK] Found {len(processed_ids)} processed articles.")
        except Exception as e:
            print(f"[Warn] Could not check old data: {e}")
    else:
        print("[Warn] Full Refresh Mode: Deleting all old data...")
        qdrant.delete_all()
    ...
    cursor = conn.cursor()
    cursor.execute(
        'SELECT id, title, content, source, published_date, link, category '
    ...
    if total == 0:
        print("\n[OK] No new articles to process.")
        return

    print(f"\n[Stats] Total new articles: {total}")
    print(f"[Config] Chunking: Recursive Character (Size=500, Overlap=100)")
    ...
    for idx, article in enumerate(new_articles, 1):
        ...
        link = article['link']
        category = article.get('category', 'Chưa phân loại')
        ...
        # 2. Vectorization
        embeddings = vectorizer.encode_batch(chunks)
        ...
        for chunk in chunks:
            ...
            # Chuyển ngày sang số YYYYMMDD để filter nhanh chính xác bằng Qdrant native filter
            pub_date_str = str(article['published_date'] or "")
            pub_date_int = 0
            if pub_date_str:
                date_match = re.search(r'(\d{4})-(\d{2})-(\d{2})', pub_date_str)
                if date_match:
                    pub_date_int = int(date_match.group(1) + date_match.group(2) + date_match.group(3))
        ...
        if idx % 10 == 0:
            print(f"  [OK] Processed {idx}/{total} articles ({total_chunks} chunks)")
    ...
    print("--- KET QUA XU LY ---")
    print("=" * 60)
    print(f"  Bai viet moi:      {total}")
    print(f"  Tong so chunks:    {total_chunks}")
    print(f"  [Time] Duration:         {duration:.2f}s ({duration/total:.2f}s/article)")
    print(f"  Database:          Qdrant (Local Persistent)")
    print("=" * 60)


def demo_search(qdrant_service: QdrantService):
    """Demo tìm kiếm với Qdrant."""
    print("\n[Search] DEMO: SEMANTIC SEARCH (QDRANT)")
    print("=" * 60)
    ...
    if qdrant.count() == 0:
        print("[Warn] DB is empty. Run process_all_articles() first.")
        return
...
def main():
    print("\n[Pipeline] RAG PIPELINE (Qdrant + Vietnamese Bi-Encoder)")
    
    # 1. Khởi tạo Qdrant Service một lần duy nhất
    from chatbot_api.services.embedding_service import EmbeddingService
    emb_service = EmbeddingService()
    qdrant_service = QdrantService(vector_size=emb_service.dimension)
    
    # 2. Chạy vector hóa
    process_all_articles(incremental=True, qdrant_service=qdrant_service, embedding_service=emb_service)
    
    # 3. Demo tìm kiếm
    demo_search(qdrant_service=qdrant_service)
